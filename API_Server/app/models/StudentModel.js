...
Student.remove = (id, result) => {
    ...
    // sql.query("SELECT * FROM students", (errall, resall) => {
    //     if (errall) {
    //         console.log("error: ", errall);
    //         result(null, errall);
    //         return;
    //     }
    //     // console.log("students: ", resall);
    //     result (null, resall);
    // });
    Student.getAll((errall, data)=>{
        if (errall) {
            result(errall, null);
            return;
...
Student.getAll = result => {
    sql.query("SELECT * FROM students order by id desc", (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(null, err);
            return;
