  ...
  onSave = async (e) => {
    ...
    if(this.checkValidate() === true){
      try {
        ...
        var url = "?name="+ this.state.name + "&age="+this.state.age + "&sex=" +sex+ "&address=" + this.state.address + "&classs=" + this.state.classs +"&score="+ this.state.score;
        const response = await UserApi.insert(url);
